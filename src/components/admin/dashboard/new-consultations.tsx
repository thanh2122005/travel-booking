  ...
  return (
    <article className="iv-card rounded-2xl border-slate-200 bg-white p-5">
      <div className="mb-3 flex items-center justify-between gap-2">
        <h3 className="text-base font-semibold text-slate-500">Yêu cầu tư vấn mới</h3>
          ...
          {items.slice(0, 7).map((inquiry) => (
            <article key={inquiry.id} className="rounded-xl border border-slate-200 bg-white p-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <p className="text-sm font-medium text-slate-500">{inquiry.fullName}</p>
              ...
              <p className="mt-1 line-clamp-1 text-sm text-slate-500">{inquiry.email} • {inquiry.phone}</p>
