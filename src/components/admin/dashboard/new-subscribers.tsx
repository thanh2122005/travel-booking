  ...
  return (
    <article className="iv-card rounded-2xl border-slate-200 p-5">
      <div className="mb-3 flex items-center justify-between gap-2">
        <h3 className="text-base font-bold text-slate-800">Email đăng ký mới</h3>
        <Link href="/admin/newsletter" className="text-sm font-semibold text-cyan-700 hover:text-cyan-800">
          Xem tất cả
        </Link>
      </div>
        ...
        <div className="space-y-3">
          {items.map((subscriber) => (
            <article key={subscriber.id} className="rounded-xl border border-slate-200 bg-white px-3 py-2.5">
              <p className="truncate text-sm font-semibold text-slate-800">{subscriber.email}</p>
              <p className="mt-1 text-xs text-slate-500">Đăng ký ngày {formatDate(subscriber.createdAt)}</p>
            </article>
          ))}
        </div>
      ) : (
