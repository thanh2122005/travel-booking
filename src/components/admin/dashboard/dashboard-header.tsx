  ...
  return (
    <section className="iv-card rounded-2xl border-slate-200/80 bg-white p-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-slate-600">Tổng quan vận hành</h1>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-3">
        {quickStats.map((item) => (
          <article key={item.label} className="rounded-xl border border-slate-200 bg-slate-50/70 px-3 py-2.5">
            ...
            <p className="mt-1 text-xl font-semibold text-slate-600">{item.value}</p>
          </article>
        ))}
      </div>
    </section>
